	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" && tInv === "invLeft"){
				...
				$.post("http://warehouse/Take",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://warehouse/Store",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://warehouse/Update",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
...
const Request = () => {
	$.post("http://warehouse/Request",JSON.stringify({}),(data) => {
		$("#weightTextLeft").html(`${(data["invPeso"]).toFixed(2)}   /   ${(data["invMaxpeso"]).toFixed(2)}`);
		$("#weightTextRight").html(`${(data["warehousePeso"]).toFixed(2)}   /   ${(data["warehouseMaxpeso"]).toFixed(2)}`);

		$("#weightBarLeft").html(`<div id="weightContent" style="width: ${data["invPeso"] / data["invMaxpeso"] * 100}%"></div>`);
		$("#weightBarRight").html(`<div id="weightContent" style="width: ${data["warehousePeso"] / data["warehouseMaxpeso"] * 100}%"></div>`);
		...
		for (let x = 1; x <= 100; x++){
			...
			if (data.myWarehouse[slot] !== undefined){
				const v = data.myWarehouse[slot];
				var maxDurability = 86400 * v["days"];
				var newDurability = (maxDurability - v["durability"]) / maxDurability;
				var actualPercent = newDurability * 100;
