...
AddEventHandler("cemitery:initBody",function()
	...
	if not Peds then
		...
		if LoadModel(List[Hash]) then
			Peds = CreatePed(4,List[Hash],Locates[Selected][1],Locates[Selected][2],Locates[Selected][3] - 1,Locates[Selected][4] - 180.0,false,false)
			...
			if LoadAnim("dead") then
				TaskPlayAnim(Peds,"dead","dead_a",8.0,8.0,-1,1,0,0,0,0)
			end

			exports["target"]:AddCircleZone("Cemitery:"..Selected,vec3(Locates[Selected][1],Locates[Selected][2],Locates[Selected][3]),0.5,{
