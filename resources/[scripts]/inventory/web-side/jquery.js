...
const updateDrag = () => {
	...
	$(".empty").droppable({
		...
		drop: function(event,ui){
			...
			$(".populated, .empty, .use, .send, .deliver, .repair").off("draggable droppable");
	...
	$(".repair").droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://inventory/Repair",JSON.stringify({
				slot: itemData.slot
			}));
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte"){
				...
				var idPort = $(this).attr("data-idPort");
				var idBlood = $(this).attr("data-idBlood");
				var Passport = $(this).attr("data-Passport");
				var idLocate = $(this).attr("data-idLocate");
				var idPremium = $(this).attr("data-idPremium");
				var idCriminal = $(this).attr("data-idCriminal");

				contents = `<item>${name} - ${Passport}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r><br>Tipo Sangüineo: <r>${idBlood}</r><br>Nacionalidade: <r>${idLocate}</r><br>Porte de Armas: <r>${idPort}</r><br>Período Criminal: <r>${idCriminal}</r><br>Premium: <r>${idPremium}</r></legenda>`;
			}
			...
			if (name == "Distintivo"){
				...
				contents = `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r><br>Tipo Sangüineo: <r>${idBlood}</r></legenda>`;
			}
