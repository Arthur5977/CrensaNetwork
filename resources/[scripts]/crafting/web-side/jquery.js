...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			$(this).tooltip({
				content: `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Economia: <r>$${economy}</r> <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r></legenda><br><legenda>${recipe}</legenda>`,
	...
	$.post("http://crafting/requestCrafting",JSON.stringify({ craft: selectCraft }),(data) => {
		...
		for (let x = 1; x <= data["invMaxpeso"]; x++){
			...
			if (data.inventario[slot] !== undefined){
				...
				const item = `<div class="item populated" style="background-image: url('nui://vrp/config/inventory/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}" data-max="${v["max"]}" data-economy="${v["economy"]}">
					<div class="top">
		...
		for (let x = 1; x <= 50; x++){
			...
			if (nameList2[x - 1] !== undefined){
				...
				const item = `<div class="item populated" title="" style="background-image: url('nui://vrp/config/inventory/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-list="${list}" data-slot="${slot}" data-description="${v["desc"]}" data-max="${v["max"]}" data-economy="${v["economy"]}">
					<div class="top">
