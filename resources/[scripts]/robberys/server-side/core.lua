			...
			if GlobalState["Blackout"] == 1 then
				if not Robberys[Number]["avaiable"] then
					...
					if os.time() >= Robberype[Robberys[Number]["type"]] then
							...
							if Consult[1] >= Robberys[Number]["need"]["amount"] then
								if not vRP.CheckDamaged(Consult[2]) then
									if vRP.TakeItem(Passport,Consult[2],Robberys[Number]["need"]["amount"]) then
										...
										TriggerClientEvent("Notify",source,"Sucesso","Progresso de desencriptação do sistema iniciado, o mesmo vai estar concluído em <b>"..Robberys[Number]["duration"].."</b> segundos.","verde",10000)
									end
								else
									TriggerClientEvent("Notify",source,"Aviso","<b>"..itemName(Robberys[Number]["need"]["item"]).."</b> danificado.","vermelho",5000)
								end
							else
								TriggerClientEvent("Notify",source,"Atenção","Precisa de <b>"..Robberys[Number]["need"]["amount"].."x "..itemName(Robberys[Number]["need"]["item"]).."</b>.","amarelo",5000)
							end
						else
						...
						TriggerClientEvent("Notify",source,false,"Cofre está vazio, aguarde <b>"..Cooldown.."</b> segundos.","azul",5000)
					end
				else
					if os.time() >= Robberys[Number]["timavaiable"] then
						...
						TriggerClientEvent("Notify",source,false,"Desencriptação em andamento, aguarde <b>"..Cooldown.."</b> segundos.","azul",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"Atenção","Você não pode fazer isso com as luzes acessas.","amarelo",5000)
			end
		end
	end
end)
