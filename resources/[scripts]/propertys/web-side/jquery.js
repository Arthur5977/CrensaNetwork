$(document).ready(function(){
	window.addEventListener("message",function(event){
		switch(event["data"]["Action"]){
			...
			case "Weight":
				$("#weightTextLeft").html(`${(event["data"]["InvPeso"]).toFixed(2)}   /   ${(event["data"]["InvMax"]).toFixed(2)}`);
				$("#weightTextRight").html(`${(event["data"]["ChestPeso"]).toFixed(2)}   /   ${(event["data"]["ChestMax"]).toFixed(2)}`);

				$("#weightBarLeft").html(`<div id="weightContent" style="width: ${event["data"]["InvPeso"] / event["data"]["InvMax"] * 100}%"></div>`);
				$("#weightBarRight").html(`<div id="weightContent" style="width: ${event["data"]["ChestPeso"] / event["data"]["ChestMax"] * 100}%"></div>`);
			break;
		}
	});
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" && tInv === "invLeft"){
				...
				$.post("http://propertys/Take",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://propertys/Store",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				...
				$.post("http://propertys/Update",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
...
const Request = () => {
	$.post("http://propertys/Request",JSON.stringify({}),(data) => {
		$("#weightTextLeft").html(`${(data["InvPeso"]).toFixed(2)}   /   ${(data["InvMax"]).toFixed(2)}`);
		$("#weightTextRight").html(`${(data["ChestPeso"]).toFixed(2)}   /   ${(data["ChestMax"]).toFixed(2)}`);

		$("#weightBarLeft").html(`<div id="weightContent" style="width: ${data["InvPeso"] / data["InvMax"] * 100}%"></div>`);
		$("#weightBarRight").html(`<div id="weightContent" style="width: ${data["ChestPeso"] / data["ChestMax"] * 100}%"></div>`);
